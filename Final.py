...
S3_WEB_REPORT_BUCKET = "aws-csp-serverless"
...
def get_cred_report():
    ...
    readerstr = response['Content'].decode("utf-8")
    # print(readerstr)
    reader = csv.DictReader(readerstr.splitlines(), delimiter=',')
    # print(reader)
    for row in reader:
       report.append(row)
...
def s3report(htmlReport, account):
    ...
    with tempfile.NamedTemporaryFile(delete=False) as f:
        # res = bytes(htmlReport, 'utf-8')
        # res = htmlReport.encode('utf-8')
        print(type(htmlReport))
